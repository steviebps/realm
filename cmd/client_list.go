...
var clientList = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if err := utils.ReadInterfaceWith(res.Body, &httpRes); err != nil {
			log.Error().Str("error", err.Error()).Msg(fmt.Sprintf("could not read response for listing: %q", args[0]))
			return err
		}

		if len(httpRes.Errors) > 0 {
			log.Error().Msg(fmt.Sprintf("could not list %q: %s", args[0], httpRes.Errors))
